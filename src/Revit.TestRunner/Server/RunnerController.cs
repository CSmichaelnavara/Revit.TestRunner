...
namespace Revit.TestRunner.Server
{
    ...
    public class RunnerController
    {
        ...
        private AssemblyResolver mAssemblyResolver;
        ...
        private void Initialize()
        {
            ...
            mAssemblyResolver = new AssemblyResolver();
            mAssemblyResolver.Start();
        }
        ...
        private ExploreResponseDto ProcessExplore(ExploreRequestDto request)
        {
            ...
            mAssemblyResolver.AddAssembly(request.AssemblyPath);

            var requestDirectory = CreateRequestDirectory(request, ExplorePath);

            var runner = new NUnitRunner(request.AssemblyPath, requestDirectory.FullName);
            var message = runner.ExploreAssembly();
        ...
        {
            foreach (var testCaseDto in request.Cases) mAssemblyResolver.AddAssembly(testCaseDto.AssemblyPath);
            ...
            LogSummary(summaryFile,
                $"Test Request '{request.RequestId}' - {request.ClientName} ({request.ClientVersion})");

            var testRunStateDto = new TestRunStateDto
            {
            ...
            var runnerTask = new RevitTask();
            var runner = new ReflectionRunner(mUiApplication);
            ...
            runnerTask.Run(async uiApplication =>
            {
                try
                {
                    ...
                    var duplicatesIds = casesToRun.GroupBy(x => x.Id)
                        .Where(g => g.Count() > 1)
                        .Select(x => x.Key);

                    if (duplicatesIds.Any())
                        throw new ArgumentException($"Case Id must be unique! {string.Join(",", duplicatesIds)}");
                ...
                LogSummary(summaryFile,
                    $"Test run end - duration {testRunStateDto.Timestamp - testRunStateDto.StartTime}");
            });
        ...
        {
            var writer = new ResultXmlWriter(resultXmlFile);
